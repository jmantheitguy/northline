...
Teams are a separate, reusable membership boundary. Each team has one owner (leader), optional managers, and members. Managers may maintain ordinary membership but cannot appoint or remove managers; only the owner can change team settings, delete the team, or connect a workspace. A team may connect to an owner-controlled shared workspace with viewer or editor access. Board discovery, board authorization, assignment eligibility, search, My Work, notification recipients, and workspace navigation all honor that inherited team access. Direct board and workspace shares remain independent and are never removed by adding a team link.
