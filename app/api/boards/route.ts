...
export async function POST(request:Request){const user=await currentUser();if(!user)return NextResponse.json({error:"Unauthorized"},{status:401});const{name,description=""}=await request.json();const cleanName=String(name||"").trim(),cleanDescription=String(description||"").trim();if(!cleanName)return NextResponse.json({error:"Board name is required"},{status:400});if(cleanName.length>100||cleanDescription.length>1000)return NextResponse.json({error:"Board details are too long"},{status:400});const result=db.prepare("INSERT INTO boards(name,description,owner_id) VALUES(?,?,?)").run(cleanName,cleanDescription,user.id);db.prepare("INSERT INTO audit_log(actor_id,action,target) VALUES(?,?,?)").run(user.id,"BOARD.CREATE",String(result.lastInsertRowid));return NextResponse.json({id:Number(result.lastInsertRowid)},{status:201});}
