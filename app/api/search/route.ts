...
export async function GET(request:Request){
  ...
  const results=await db.prepare(`SELECT DISTINCT t.id,t.title,t.status,t.priority,b.id boardId,b.public_id boardKey,b.name boardName,t.updated_at AS "updatedAt"
    ...
    ORDER BY "updatedAt" DESC LIMIT 25`).all(user.id,user.id,user.id,user.id,user.id,user.id,user.id,user.id,user.id,...Array(3).fill(`%${q.replace(/[\\%_]/g,"\\$&")}%`));
