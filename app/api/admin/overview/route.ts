...
async function describeAudit(admin: NonNullable<Awaited<ReturnType<typeof requireAdmin>>>, item: AuditRecord) {
  ...
  if (item.action.startsWith("TASK.")) {
    const task = await db.prepare("SELECT id,title,board_id boardId FROM tasks WHERE id=?").get(Number(rawId)) as { id:number; title:string; boardId:number } | undefined;
    return { ...item, label, description: task && await boardPermission(admin, task.boardId) ? (item.detail || `${label} “${task.title}”`) : `${label} on a private or removed board` };
  }
  if (item.action.startsWith("BOARD.")) {
    const board = await db.prepare("SELECT id,name FROM boards WHERE id=? OR public_id=?").get(Number(rawId) || -1, rawId) as { id:number; name:string } | undefined;
    return { ...item, label, description: board && await boardPermission(admin, board.id) ? (item.detail || `${label} “${board.name}”`) : `${label} on a private or removed board` };
  }
  if (item.action.startsWith("CALENDAR.")) {
    const calendarId = Number(rawId);
    return { ...item, label, description: calendarId && await calendarPermission(admin, calendarId) ? (item.detail || label) : `${label} on a private or removed calendar` };
  }
  if (item.detail) return { ...item, label, description: item.detail };
  if (item.action.startsWith("USER.")) {
    const target = await db.prepare("SELECT name,email FROM users WHERE id=? OR LOWER(email)=LOWER(?)").get(Number(rawId) || -1, rawId) as { name:string; email:string } | undefined;
...
export async function GET() {
  ...
  const metrics={
    users:Number((await db.prepare("SELECT COUNT(*) count FROM users").get() as {count:number}).count),
    activeUsers:Number((await db.prepare("SELECT COUNT(*) count FROM users WHERE status='Active'").get() as {count:number}).count),
    activeBoards:Number((await db.prepare("SELECT COUNT(*) count FROM boards").get() as {count:number}).count),
    workspaces:Number((await db.prepare("SELECT COUNT(*) count FROM workspaces").get() as {count:number}).count),
    tasks:Number((await db.prepare("SELECT COUNT(*) count FROM tasks WHERE archived_at IS NULL").get() as {count:number}).count),
    activeTimers:Number((await db.prepare("SELECT COUNT(*) count FROM time_entries WHERE ended_at IS NULL AND deleted_at IS NULL").get() as {count:number}).count),
    failedReminders:Number((await db.prepare("SELECT COUNT(*) count FROM reminders WHERE status='failed'").get() as {count:number}).count),
