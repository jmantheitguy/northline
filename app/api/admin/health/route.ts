...
export async function GET(){
  ...
  return NextResponse.json({generatedAt:new Date().toISOString(),application:{version:process.env.npm_package_version||"0.7.0-alpha.0",uptimeSeconds:Math.round(process.uptime()),rssBytes:memory.rss,heapBytes:memory.heapUsed,node:process.version},database:{status:integrity==="ok"?"healthy":"degraded",integrity,sizeBytes:fs.statSync(databasePath).size},storage:{status:disk.bavail/disk.blocks>.1?"healthy":"degraded",freeBytes:disk.bavail*disk.bsize,totalBytes:disk.blocks*disk.bsize},identity:{status:process.env.NORTHLINE_OIDC_ISSUER?"configured":"local-only",activeSessions:sessions},discord,reminders:{counts:Object.fromEntries(reminders.map(item=>[item.status,item.count])),lastDelivery:delivery},backup:statusFile("backup"),restore:statusFile("restore")});
}
