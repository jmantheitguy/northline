...
import { discordConfigured,listDiscordChannels,sendDiscordDirectMessage } from "@/lib/discord";
...
export async function GET(){
  ...
  return NextResponse.json({generatedAt:new Date().toISOString(),application:{version:process.env.npm_package_version||"0.2.0-beta.0",uptimeSeconds:Math.round(process.uptime()),rssBytes:memory.rss,heapBytes:memory.heapUsed,node:process.version},database:{status:integrity==="ok"&&migration.version>=11?"healthy":"degraded",integrity,sizeBytes:fs.statSync(databasePath).size,migrationVersion:migration.version,migrationsApplied:migration.count},storage:{status:disk.bavail/disk.blocks>.1?"healthy":"degraded",freeBytes:disk.bavail*disk.bsize,totalBytes:disk.blocks*disk.bsize},identity:{status:process.env.NORTHLINE_OIDC_ISSUER?"configured":"local-only",activeSessions:sessions},discord,reminders:{counts:Object.fromEntries(reminders.map(item=>[item.status,item.count])),lastDelivery:delivery},backup,restore});
}
export async function POST(){
  ...
  const recipient=db.prepare("SELECT discord_user_id discordUserId FROM users WHERE id=?").get(admin.id) as {discordUserId:string|null}|undefined;
  if(!recipient?.discordUserId)return NextResponse.json({error:"Link Discord before testing Task Buddy delivery"},{status:409});
  await sendDiscordDirectMessage(recipient.discordUserId,`🩺 **Northline health check**\n✅ **${admin.name}** verified private Task Buddy delivery.\n${(process.env.NORTHLINE_PUBLIC_URL||"https://northline.vtuberoffices.com").replace(/\/$/,"")}`);
  db.prepare("INSERT INTO audit_log(actor_id,action,target) VALUES(?,?,?)").run(admin.id,"HEALTH.DISCORD.TEST","Direct message");
