...
export async function GET(){
  ...
  return NextResponse.json({generatedAt:new Date().toISOString(),application:{version:process.env.npm_package_version||"0.0.1-beta.0",uptimeSeconds:Math.round(process.uptime()),rssBytes:memory.rss,heapBytes:memory.heapUsed,node:process.version},database:{status:integrity==="ok"&&migration.version>=9?"healthy":"degraded",integrity,sizeBytes:fs.statSync(databasePath).size,migrationVersion:migration.version,migrationsApplied:migration.count},storage:{status:disk.bavail/disk.blocks>.1?"healthy":"degraded",freeBytes:disk.bavail*disk.bsize,totalBytes:disk.blocks*disk.bsize},identity:{status:process.env.NORTHLINE_OIDC_ISSUER?"configured":"local-only",activeSessions:sessions},discord,reminders:{counts:Object.fromEntries(reminders.map(item=>[item.status,item.count])),lastDelivery:delivery},backup,restore});
}
