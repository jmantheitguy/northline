...
export async function PATCH(request: Request) {
  ...
  await db.prepare("UPDATE users SET timezone=? WHERE id=?").run(timezone, user.id);
  const dueTasks=await db.prepare(`SELECT t.id,t.board_id boardId,t.title,t.status,t.assignee_id assigneeId,t.due_date dueDate,t.created_by createdBy
  ...
  for(const task of dueTasks)await scheduleDueNotification(task,user.id);
