...
export async function GET() {
  ...
  const reminders=db.prepare(`SELECT r.id,r.board_id boardId,r.task_id taskId,r.channel_id channelId,r.channel_name channelName,
    ...
    WHERE b.owner_id=? OR w.owner_id=? OR bm.user_id=? OR wm.user_id=? ORDER BY r.remind_at DESC LIMIT 100`).all(user.id,user.id,user.id,user.id,user.id,user.id,user.id,user.id) as Array<Record<string,unknown>>;
...
export async function POST(request:Request) {
  ...
  const recipientId=task?.createdBy||user.id;const recipient=db.prepare("SELECT discord_user_id discordUserId FROM users WHERE id=?").get(recipientId) as {discordUserId:string|null}|undefined;
  if(!recipient?.discordUserId)return NextResponse.json({error:"The reminder recipient has not linked Discord"},{status:409});
  const result=db.prepare("INSERT INTO reminders(board_id,task_id,created_by,recipient_user_id,channel_id,channel_name,message,remind_at) VALUES(?,?,?,?,?,?,?,?)")
    .run(Number(boardId),taskId?Number(taskId):null,user.id,recipientId,"dm","Direct message",String(message).trim(),when.toISOString());
