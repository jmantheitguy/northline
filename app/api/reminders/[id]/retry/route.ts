...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}) {
  ...
  db.prepare("UPDATE reminders SET status='pending',remind_at=CURRENT_TIMESTAMP,error=NULL,sent_at=NULL WHERE id=?").run(id);
  db.prepare("INSERT INTO audit_log(actor_id,action,target) VALUES(?,?,?)").run(user.id,"REMINDER.RETRY",String(id));
