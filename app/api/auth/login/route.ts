...
export async function POST(request:Request) {
  ...
  const user=db.prepare("SELECT id,name,email,role,status,password_hash,timezone FROM users WHERE email=?").get(String(email||"")) as {id:number;name:string;email:string;role:string;status:string;password_hash:string;timezone:string}|undefined;
